 ...
 * @brief Call AuditLogManager's Submit request and return a unique identifier
    ...
    )
{
    ...
    try
    {
        ...
        if ((nullptr != c_pbSerializedRequest)&&(0 < unSerializedRequestSizeInBytes)&&(nullptr != punSerializedResponseSizeInBytes))
        {
            ...
            un64Identifier = poAuditLogManager->SubmitRequest(oRequest, punSerializedResponseSizeInBytes);
        }
 ...
 * @brief Call AuditLogManager's GetResponse and return response associated
 ...
 *        function. This function creates  singleton object of AuditLogManager class and
 *        calls AuditLogManager's InitializePlugin, which initializes the plugin's
 *        data members including its dictionary. This function then calls RegisterPluginFn which is used to
 *        register AuditLogManager's dictionary with the RestFramework
 ...
 * @brief Call AuditLogManager to release the object resources and
