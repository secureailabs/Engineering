    ...
    )
{
    ...
    m_qw64BitHash = 0;
    m_qwComposition64BitHash = 0;
    this->DeSerialize(c_oSerializedBuffer.data(), c_oSerializedBuffer.size());
}
...
std::string __thiscall StructuredBuffer::GetBase64SerializedBuffer(void) const throw()
{
    ...
    return ::Base64Encode(m_stlSerializedBuffer.data(), m_stlSerializedBuffer.size());
}
    ...
    try
    {
        ...
        if ((0 == m_qw64BitHash)||(0 == m_qwComposition64BitHash)||(0 == m_stlSerializedBuffer.size()))
        {
            ...
            for (auto const & vectorElement : stlVectorOfElementName64BitHashes) 
            {
                ...
                unSerializedBufferSizeInBytes += sizeof(Dword) + sizeof(uint32_t) + stlSerializedElements[vectorElement].size();
            }
    ...
    )
{
    ...
    this->DeSerialize(stlRawBuffer.data(), stlRawBuffer.size());
}
