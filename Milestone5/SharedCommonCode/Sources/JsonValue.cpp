...
std::string __thiscall JsonValue::ToString(void) const
{
    __DebugFunction();
...
JsonValue * __thiscall JsonValue::ToJson(
    _in const char ** c_pszData
    )
{
    __DebugFunction();
    ...
    else if ((4 == strnlen(*c_pszData, 4)) && (0 == strncasecmp(*c_pszData, "true", 4)))
    {
        (*c_pszData) += 4;
    ...
    else if ((5 == strnlen(*c_pszData, 5)) && (0 == strncasecmp(*c_pszData, "false", 5)))
    {
        (*c_pszData) += 5;
    ...
    // Check if null
    else if ((4 == strnlen(*c_pszData, 4)) && (0 == strncasecmp(*c_pszData, "null", 4)))
    {
        (*c_pszData) += 4;
    ...
    )
{
    ...
    while (itrStructuredBufferElements != stlStructuredBufferElements.end())
    {
        ...
        unsigned int unPositionOfEndOfType = (*itrStructuredBufferElements).find("],Size[");
...
JsonValue * __stdcall JsonValue::ParseObject(
    _in const char ** c_pszData
    )
{
    __DebugFunction();
