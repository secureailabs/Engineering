...
void __thiscall StructuredBufferElement::Serialize(void) const throw()
{
    ...
    unsigned int unSerializedBufferSizeInBytes = (unsigned int) (sizeof(Dword) + sizeof(Byte) + sizeof(uint32_t) + sizeof(uint32_t) + sizeof(Dword) + m_strElementName.length() + 1 + sizeof(Dword) + m_stlElementData.size() + sizeof(Dword));
