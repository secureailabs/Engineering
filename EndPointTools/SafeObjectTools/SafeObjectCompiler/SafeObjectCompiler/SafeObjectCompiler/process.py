...
class TemplateProcessor:
    ...
    def ProcessAll(self, substituteContents):
        ...
        self.WriteStructureBuffer(safeFunction, substituteContents.libdict, self.destination)
    ...
    def ProcessImports(self, importContents):
        ...
        for contentBlock in importContents:
            substituteImportContents = substituteImportContents + contentBlock.blockToString(self.lines)
    ...
    def ProcessClasses(self, classContents):
        ...
        for contentBlock in classContents:
            substituteClassContents = substituteClassContents + contentBlock.blockToString(self.lines)
    ...
    def ProcessFunctions(self, functionContents):
        ...
        for contentBlock in functionContents:
            if contentBlock.name == "Run":
                majorFunction = majorFunction + contentBlock.blockToStringMajor(self.lines)
                entryFunction = self.GenerateMainFunction(contentBlock)
            else:
                localFunctions = localFunctions + contentBlock.blockToString(self.lines)
    ...
    def GenerateMainFunction(self, funcBlock):
        ...
        if isinstance(returnValues, typing._GenericAlias):
            if isinstance(returnValues.__origin__, tuple):
                res = []
                for item in returnValues.__args__:
                    res.append(item)
                returnValues = res
            else:
                returnValues = [returnValues.__origin__]
        else:
            returnValues = [returnValues]

        returnConfidentiality = []
        if funcBlock.doc is None:
            raise Exception("Need to specify confidentiality in the docstring")
        docstrings = funcBlock.doc.splitlines()
        flag = False
        for line in docstrings:
            line = line.replace(" ", "")
            if line.startswith("Confidentiality"):
                flag = True
                line = line[16:].split(",")
                for props in line:
                    if props == "True":
                        returnConfidentiality.append(True)
                    elif props == "False":
                        returnConfidentiality.append(False)
                    else:
                        raise Exception("Confidentiality can only be True or False")
                break
        if not flag:
            raise Exception("Confidentiality not specified in the docstring")

        return SafeFunction(self.title, funcBlock.doc, args, returnValues, returnConfidentiality)
    ...
    def GenerateExecution(self, safeFunction):
        ...
        for returnid in safeFunction.returnsuuid:
            resultValues = resultValues + f"safe{safeFunction.uuid}.m_{returnid}, "
        resultValues = resultValues[:-2]
        executionStr = f"{resultValues} = safe{safeFunction.uuid}.Run({inputArgs})"
